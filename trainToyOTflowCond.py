...
if __name__ == '__main__':
    ...
    xy = toy_data.inf_train_gen(args.data, batch_size=args.batch_size)
    xy = cvt(torch.from_numpy(xy))
    print(xy.shape)
    x0 = xy[:,0].view(-1,dx)
    y  = xy[:,1].view(-1,dy)
    ...
    xyval = toy_data.inf_train_gen(args.data, batch_size=args.val_batch_size)
    xyval = cvt(torch.from_numpy(xyval))
    x0val = xyval[:,0].view(-1,dx)
    yval = xyval[:,1].view(-1,dy)
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.val_freq == 0 or itr == args.niters:
            with torch.no_grad():
                net.eval()
                test_loss, test_costs = compute_loss(net, x0val, yval, nt=nt_val)
        ...
        if itr % args.viz_freq == 0:
            with torch.no_grad():
                ...
                plot4(net, x_samples,y_samples, z, nt_val, sPath, doPaths=True, sTitle='{:s}  -  loss {:.2f}  ,  C {:.2f}  ,  alph {:.1f} {:.1f}  '
