...
def plot4(net, x, y, z, nt_val, sPath, sTitle="", doPaths=False):
    ...
    im1 , _, _, map1 = axs[0, 0].hist2d(x.detach().cpu().numpy()[:, d1], y.detach().cpu().numpy()[:, d2], range=[[LOWX, HIGHX], [LOWY, HIGHY]], bins=nBins)
    axs[0, 0].set_title('x from rho_0')
    im2 , _, _, map2 = axs[0, 1].hist2d(fx.detach().cpu().numpy()[:, d1], y.detach().cpu().numpy()[:, d2], range=[[-4, 4], [-4, 4]], bins = nBins)
    axs[0, 1].set_title('f(x)')
    im3 , _, _, map3 = axs[1, 0].hist2d(finvfx.detach().cpu().numpy()[: ,d1] ,y.detach().cpu().numpy()[: ,d2], range=[[LOWX, HIGHX], [LOWY, HIGHY]], bins = nBins)
    axs[1, 0].set_title('finv( f(x) )')
    im4 , _, _, map4 = axs[1, 1].hist2d(genModel.detach().cpu().numpy()[:, d1], y.detach().cpu().numpy()[:, d2], range=[[LOWX, HIGHX], [LOWY, HIGHY]], bins = nBins)
    ...
    if False and doPaths:
        forwPath = integrate(x[:, 0:dx].view(-1,dx), y, net, [0.0, 1.0], nt_val, stepper="rk4", alph=net.alph, intermediates=True)
        backPath = integrate(fx[:, 0:dx].view(-1,dx),y , net, [1.0, 0.0], nt_val, stepper="rk4", alph=net.alph, intermediates=True)
...
def plotAutoEnc(x, xRecreate, sPath):
    print(x.shape, xRecreate.shape, '   x, xRecreate')
    # assume square image
    s = int(math.sqrt(x.shape[1]))

    xRecreate = xRecreate[:, :s*s]
    nex = 8
