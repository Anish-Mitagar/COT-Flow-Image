...
parser.add_argument('--autoenc'    , type=str, default='/home/yding37/Documents/project_n/OT-Flow/experiments/cnf/large/2024_06_20_02_45_02_autoenc_checkpt.pth') #'/home/yding37/Documents/project_n/OT-Flow/experiments/cnf/large/2024_06_17_12_22_03_autoenc_checkpt.pth'
...
if __name__ == '__main__':
    ...
    while itr < args.niters:
        ...
        for data in train_loader:
            ...
            # # visualize y (for debugging)
            # plt.imshow(images[0, 0].cpu().numpy(), cmap='gray')
            # plt.savefig('output.png')

            x0 = cvt(x0)
            y = cvt(y)
            combined_images = torch.cat((images, conditions), dim=1)
            combined_images = combined_images.view(combined_images.size(0), -1)
            combined_images = cvt(combined_images)

            encoded_tensor = autoEnc.encode(combined_images)
            normalized_tensor = (encoded_tensor - autoEnc.mu) / (autoEnc.std + args.eps)
            ...
            if itr == 1 or itr % args.val_freq == 0 or itr == args.niters:
                ...
                with torch.no_grad():
                    ...
                    for data in val_loader:
                        ...
                        encoded_tensor = autoEnc.encode(combined_images)
                        # 
                        # input_tensor = torch.cat((x0, y), dim=1)
                        # encoded_tensor = autoEnc.encode(x0, y)
                        normalized_tensor = (encoded_tensor - autoEnc.mu) / (autoEnc.std + args.eps)
                        x0, y = normalized_tensor.chunk(2, dim=1)
